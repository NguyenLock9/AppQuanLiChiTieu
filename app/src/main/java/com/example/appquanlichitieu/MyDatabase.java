...
public class MyDatabase {
    ...
    @SuppressLint("Range")
    public List<GiaoDich> getAllDanhMucGiaoDichThu(Context context) {
        List<GiaoDich> list = new ArrayList<>();
        DBHelper dbHelper = new DBHelper(context);
        try (SQLiteDatabase db = dbHelper.getWritableDatabase();
             Cursor cursor = db.query(DBHelper.TEN_BANG_GIAODICH, null, DBHelper.COT_ISKHOANTHU + "= ?",
                     new String[]{"1"}, null, null, null)) { // Lấy ra các giao dịch thu (COT_ISKHOANTHU = 1)
            while (cursor.moveToNext()) {
                int id = cursor.getInt(cursor.getColumnIndexOrThrow(DBHelper.COT_ID_DANHMUC));
    ...
    @SuppressLint("Range")
    public List<GiaoDich> getAllDanhMucGiaoDichChi(Context context) {
        List<GiaoDich> list = new ArrayList<>();
        DBHelper dbHelper = new DBHelper(context);
        try (SQLiteDatabase db = dbHelper.getWritableDatabase();
             Cursor cursor = db.query(DBHelper.TEN_BANG_GIAODICH, null, DBHelper.COT_ISKHOANTHU + "= ?",
                     new String[]{"0"}, null, null, null)) { // Lấy ra các giao dịch thu (COT_ISKHOANTHU = 1)
            while (cursor.moveToNext()) {
                int id = cursor.getInt(cursor.getColumnIndexOrThrow(DBHelper.COT_ID_DANHMUC));
                GiaoDich giaoDich = new GiaoDich();
                giaoDich.setId(Integer.parseInt(cursor.getString(cursor.getColumnIndex(DBHelper.COT_ID_GIAODICH))));
    ...
    public void updateGiaoDich(GiaoDich giaoDich) {
        ...
        ContentValues values = new ContentValues();
        values.put(DBHelper.COT_CATEGORY, giaoDich.getCategory());
        values.put(DBHelper.COT_AMOUNT, giaoDich.getAmount());

        database.update(DBHelper.TEN_BANG_GIAODICH, values, DBHelper.COT_ID_GIAODICH + " = ?", new String[]{String.valueOf(giaoDich.getId())});

        database.close();
    }
    public void deleteGiaoDich(GiaoDich giaoDich) {

        database.delete(DBHelper.TEN_BANG_GIAODICH,  DBHelper.COT_ID_GIAODICH + " = ?", new String[]{String.valueOf(giaoDich.getId())});
        database.close();
    }
