...
import com.example.appquanlichitieu.GiaoDichAdapterThu;
...
public class KhoanThuFragment extends Fragment implements DatePickerDialog.OnDateSetListener  {
    ...
    ImageButton btnimgDeleteDanhMuc;
    View view;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
       ...
       // Thiết lập adapter cho Spinner
        ...
        btnimgDeleteDanhMuc=view.findViewById(R.id.imgbtnDeleteDanhMuc);
        btnimgDeleteDanhMuc.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                // Xóa nội dung danh mục khỏi database
                int selectedItemPosition = spnDanhMuc.getSelectedItemPosition();

                if (selectedItemPosition != AdapterView.INVALID_POSITION) {
                    DanhMuc selectedDanhMuc = danhMucList.get(selectedItemPosition);
                    int selectedDanhMucId = selectedDanhMuc.get_id();

                    // Kết nối và xóa dữ liệu từ database
                    MyDatabase database = new MyDatabase(getContext());
                    database.XoaDanhMuc(selectedDanhMuc);

                    // Xóa nội dung danh mục khỏi danh sách danh mục và cập nhật lại danh mục adapter
                    danhMucList.remove(selectedItemPosition);
                    danhMucAdapter.notifyDataSetChanged();
...
// Khởi tạo GiaoDichAdapter
        GiaoDichAdapterThu adapter = new GiaoDichAdapterThu(getActivity(), giaoDichs);
...
// Thiết lập adapter cho listView
    ...
    public void setListView() {
        if (giaoDichs != null) {
            listView.setAdapter(new GiaoDichAdapterThu(getContext(), giaoDichs));
